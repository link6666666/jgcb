...
import com.jgcb.mapper.KaraokeSessionMapper;
...
@Service
public class KaraokeSessionServiceImpl extends ServiceImpl<KaraokeSessionMapper, KaraokeSession> implements KaraokeSessionService {

    private final SysUserService sysUserService;
    private final ObjectMapper objectMapper;

    public KaraokeSessionServiceImpl(SysUserService sysUserService, ObjectMapper objectMapper) {
        this.sysUserService = sysUserService;
        this.objectMapper = objectMapper;
    }

    @Override
    public void create(Long userId, CreateKaraokeRequest request) {
    ...
    public void join(Long sessionId, Long userId) {
        KaraokeSession session = this.getById(sessionId);
        if (session == null) {
            throw new RuntimeException("唱歌局不存在");
        }
        ...
        if (playerIds.contains(userId)) {
            throw new RuntimeException("你已经在唱歌局中了");
        }
    ...
    public void leave(Long sessionId, Long userId) {
        ...
        if (session.getCreatorId().equals(userId)) {
            throw new RuntimeException("发起人不能退出，请取消唱歌局");
        }
        List<Long> playerIds = parsePlayerIds(session.getPlayerIds());
    ...
    @Override
    public Page<KaraokeVO> list(int page, int size) {
        Page<KaraokeSession> sessionPage = this.page(
                new Page<>(page, size),
                new LambdaQueryWrapper<KaraokeSession>()
                        .in(KaraokeSession::getStatus, 1, 2, 3)
                        .orderByDesc(KaraokeSession::getSessionTime)
        );
        return toVOPage(sessionPage);
    }

    @Override
    public KaraokeVO getDetail(Long sessionId) {
        KaraokeSession session = this.getById(sessionId);
        if (session == null) return null;
        return toVO(session);
    }

    private Page<KaraokeVO> toVOPage(IPage<KaraokeSession> page) {
        Page<KaraokeVO> voPage = new Page<>(page.getCurrent(), page.getSize(), page.getTotal());
        voPage.setRecords(page.getRecords().stream().map(this::toVO).collect(Collectors.toList()));
        return voPage;
    }

    private KaraokeVO toVO(KaraokeSession session) {
        KaraokeVO vo = new KaraokeVO();
