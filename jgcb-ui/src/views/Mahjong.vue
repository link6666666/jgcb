<template>
  <div class="page-content">
    <div class="section-title">🀄 约麻将</div>
    ...
    <div v-if="showForm" class="publish-card">
      <div class="form-item">
        <label>标题</label>
        <input v-model="form.title" placeholder="三缺一速来" />
      </div>
      <div class="form-item">
        <label>地点</label>
        <select v-model="form.location">
          <option value="">请选择地点</option>
          <option value="龙小主">胡一天</option>
          <option value="四方城">四方城</option>
          <option value="杠上发">杠上发</option>
          <option value="其他地点">其他地点</option>
        </select>
        <input v-if="form.location === '其他地点'" v-model="form.customLocation" placeholder="请输入具体地点" />
      </div>
      <div class="form-item">
        <label>时间</label>
    ...
    <div v-if="stats.length > 0" class="stats-panel">
      ...
      <table v-if="showStats" class="stats-table">
        ...
        <tbody>
          <tr v-for="(p, i) in stats" :key="p.userId">
            <td>{{ i + 1 }}</td>
            <td class="stats-player">
              <img decoding="async" loading="lazy" :src="thumbUrl(p.avatar, 128)" class="stats-avatar" @error="$event.target.style.display='none'" />
              <span>{{ p.nickname }}</span>
            </td>
            <td>{{ p.gameCount }}</td>
            <td :class="p.totalScore > 0 ? 'score-positive' : p.totalScore < 0 ? 'score-negative' : ''">
              {{ p.totalScore > 0 ? '+' : '' }}{{ p.totalScore }}
            </td>
            <td>{{ p.avgScore > 0 ? '+' : '' }}{{ p.avgScore }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-for="s in sessions" :key="s.id" class="session-card" :class="{ cancelled: s.status === 0 }">
      ...
      <div class="sess-actions">
        ...
        <button v-if="canCancel(s)" class="btn-cancel" @click="handleCancel(s.id)">取消牌局</button>
    ...
    <div v-if="detailVisible" class="modal-overlay" @click.self="detailVisible = false">
      <div class="modal-card">
        ...
        <div class="detail-actions">
          <button v-if="detail.creatorId === currentUserId && (detail.status === 1 || detail.status === 2)" class="btn-settle" @click="openSettle">结算</button>
...
import { createSession, getSessions, joinSession, leaveSession, cancelSession, getSessionDetail, settleSession, getMahjongStats } from '../api'
...
async function loadStats() {
  try {
    const res = await getMahjongStats()
...
async function loadSessions() {
  try {
    const res = await getSessions(currentPage.value)
...
async function handleCreate() {
  ...
  const location = form.location === '其他地点' ? (form.customLocation || '其他地点') : form.location
  creating.value = true
  try {
    await createSession({ ...form, location, maxPlayers: Number(form.maxPlayers) })
    toast('牌局创建成功', 'success')
...
async function handleJoin(id) {
  ...
  const alreadyIn = s.players.some(p => p.userId === currentUserId.value)
  if (!alreadyIn) {
...
async function submitSettle() {
  settling.value = true
  try {
    await settleSession(detail.value.id, { items: settleForm.value.map(p => ({ userId: p.userId, score: p.score || 0 })) })
    toast('结算成功', 'success')
