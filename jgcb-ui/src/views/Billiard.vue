<template>
  <div class="page-content">
    <div class="section-title">🎱 约台球</div>
    ...
    <div v-if="showForm" class="publish-card">
      <div class="form-item">
        <label>标题</label>
        <input v-model="form.title" placeholder="如：约一局" />
      </div>
      <div class="form-item">
        <label>地点</label>
    ...
    <div v-for="s in sessions" :key="s.id" class="session-card" :class="{ cancelled: s.status === 0 }">
      ...
      <div class="sess-actions">
        ...
        <button v-if="canCancel(s)" class="btn-cancel" @click="handleCancel(s.id)">取消台球局</button>
...
import { createBilliard, getBilliards, joinBilliard, leaveBilliard, cancelBilliard, getBilliardDetail, settleBilliard } from '../api'
...
async function loadSessions() {
  try {
    const res = await getBilliards(currentPage.value)
...
async function handleCreate() {
  ...
  creating.value = true
  try {
    await createBilliard({ ...form, maxPlayers: Number(form.maxPlayers) })
    toast('台球局创建成功', 'success')
...
async function handleCancel(id) {
  if (!confirm('确定取消该台球局？')) return
  const s = sessions.value.find(s => s.id === id)
  if (!s) return
  const oldStatus = s.status
  s.status = 0
  try {
    await cancelBilliard(id)
    toast('台球局已取消', 'info')
  } catch (e) {
    s.status = oldStatus
...
async function showDetail(id) {
  try {
    const res = await getBilliardDetail(id)
...
async function submitSettle() {
  settling.value = true
  try {
    await settleBilliard(detail.value.id, { items: settleForm.value.map(p => ({ userId: p.userId, score: p.score || 0 })) })
    toast('结算成功', 'success')
